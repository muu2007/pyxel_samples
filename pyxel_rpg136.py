...
def btnpA(): return pyxel.btnp(pyxel.GAMEPAD_1_A) or pyxel.btnp(pyxel.KEY_1) or pyxel.btnp(pyxel.KEY_KP_1) or pyxel.btnp(pyxel.KEY_Z) or pyxel.btnp(pyxel.MOUSE_LEFT_BUTTON)
def btnpB(): return pyxel.btnp(pyxel.GAMEPAD_1_B) or pyxel.btnp(pyxel.KEY_2) or pyxel.btnp(pyxel.KEY_KP_2) or pyxel.btnp(pyxel.KEY_X) or pyxel.btnp(pyxel.MOUSE_RIGHT_BUTTON)
def blinker(): return pyxel.frame_count // 16 % 2  # 右、左、右、左、の変化用
def chip(n): return n//1024, (n % 1024 % 32)*8, (n % 1024 // 32)*8
def camera(x, y): global g_offset_x, g_offset_y; g_offset_x, g_offset_y = x, y  # 全描画命令をwrapすれば完全になるが……
...
def draw_text(x, y, text, col=7): [pyxel.pset(g_offset_x+x+u, g_offset_y+y+v, col) for u, v in Font1.points(text)]
...
def draw_frame(x, y, w, h, col=0):  # col 0: white 1:yellow 2:red x,yは枠の左上ではない。中のテキストの左上
    ...
    pyxel.blt(g_offset_x+x+w+B, g_offset_y+y-A, 0, col*8, 16, -A, A, 15)
    pyxel.blt(g_offset_x+x-A, g_offset_y+y+h+C, 0, col*8, 16, A, -A, 15)
    pyxel.blt(g_offset_x+x+w+B, g_offset_y+y+h+C, 0, col*8, 16, -A, -A, 15)
    [pyxel.blt(g_offset_x+x+i, g_offset_y+y-A, 0, col*8+7, 16, 1, A, 15) for i in range(w+B)]
    [pyxel.blt(g_offset_x+x+i, g_offset_y+y+h+C, 0, col*8+7, 16, 1, -A, 15) for i in range(w+B)]
    [pyxel.blt(g_offset_x+x-A, g_offset_y+y+j, 0, col*8, 16+7, A, 1, 15) for j in range(h+C)]
    [pyxel.blt(g_offset_x+x+w+B, g_offset_y+y+j, 0, col*8, 16+7, -A, 1, 15) for j in range(h+C)]
...
class Battle(State):
    ...
    def draw(self):
        self.original_state.draw()
        pyxel.rect((pyxel.width-64)//2+self.offset[0]+g_offset_x, (pyxel.height-64)//2+self.offset[1]+g_offset_y, 64, 64, 0)  # 背景
        pyxel.rectb((pyxel.width-64)//2+self.offset[0]+g_offset_x, (pyxel.height-64)//2+self.offset[1]+g_offset_y, 64, 64, 7)  # 背景
        draw_playerstatus()  # 下になるので再度描画
        if self.textbox:
            self.textbox.draw_just()
        if self.monster.hp > 0:
            self.monster.draw(self.offset[0]+g_offset_x, self.offset[1]+g_offset_y)  # モンスターが一番上
...
class Slime(Charactor):
    ...
    def draw(self, ox, oy): pyxel.blt((pyxel.width-16)//2+ox, (pyxel.height-16)//2+oy, 2, 0, 0, 16, 16, 15)
...
class Rabbit(Charactor):
    ...
    def draw(self, ox, oy): pyxel.blt((pyxel.width-16)//2+ox, (pyxel.height-16)//2+oy, 2, 16, 0, 16, 16, 15)
...
class Mimic(Charactor):
    ...
    def draw(self, ox, oy): pyxel.blt((pyxel.width-16)//2+ox, (pyxel.height-16)//2+oy, 2, 32, 0, 16, 16, 15)
...
class Pi(Charactor):
    ...
    def draw(self, ox, oy): pyxel.blt((pyxel.width-16)//2+ox, (pyxel.height-16)//2+oy, 2, 48, 0, 16, 16, 15)
    def choice(self): return random.choice(["たたかう"]*self.hp+(["ヒール"]*2 if self.mp > 2 else [])+(["ファイア"]*1 if self.mp > 3 else [])+(["やくそう"]*2 if "やくそう" in self.items else []))
...
class Theta(Charactor):
    ...
    def draw(self, ox, oy): pyxel.blt((pyxel.width-16)//2+ox, (pyxel.height-16)//2+oy, 2, 64, 0, 16, 16, 15)
    def choice(self): return random.choice(["たたかう"]*self.hp+(["ヒール"]*2 if self.mp > 2 else [])+(["ファイア"]*1 if self.mp > 3 else [])+(["やくそう"]*2 if "やくそう" in self.items else []))
...
class Root(Charactor):
    ...
    def draw(self, ox, oy): pyxel.blt((pyxel.width-16)//2+ox, (pyxel.height-16)//2+oy, 2, 80, 0, 16, 16, 15)
    def choice(self): return random.choice(["たたかう"]*self.hp+(["ヒール"]*2 if self.mp > 2 else [])+(["ファイア"]*1 if self.mp > 3 else [])+(["やくそう"]*2 if "やくそう" in self.items else []))
